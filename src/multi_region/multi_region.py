...
from src.utils.eval_utils import load_model_data_local, co_smoothing_eval, behavior_probe_eval
import warnings
...
ap.add_argument("--base_path", type=str, default='/expanse/lustre/scratch/zwang34/temp_project/random_exp/multi_region')
ap.add_argument("--train", action='store_true')
ap.add_argument("--eval", action='store_true')   # Not implemented
ap.add_argument("--probe", action='store_true')  # Not used
...
for cur_region in RE_SITES:
    
    # load config
    kwargs = {
        "model": "include:src/configs/multi_region/ndt1_v0.yaml"
    }
    
    config = config_from_kwargs(kwargs)
    config = update_config("src/configs/multi_region/trainer_ndt1_v0.yaml", config)
    ...
    config['data']['brain_region'] = cur_region  # only use neurons in one region
    ...
    if config.wandb.use:
        ...
        wandb.init(
            ...
            config=config,
            name="({}){}_{}_model_{}_method_{}_mask_{}_ratio_{}_ual_training_{}_{}".format(
                prefix,
                eid[:5],
                cur_region,
    ...
    if args.train:
        ...
        if num_neurons == 0:
            print(f'No neurons in {cur_region} in this session!') 
            wandb.finish()
            continue
        ...
        train_dataloader = make_loader(train_dataset, 
                                 ...
                                 brain_region=config.data.brain_region,
                                 shuffle=True,
                                )
