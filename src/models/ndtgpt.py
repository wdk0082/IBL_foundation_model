...
DEFAULT_CONFIG = "src/configs/ndtgpt/ndtgpt.yaml"


@dataclass
class NDTGPTOutput(ModelOutput):
...
class NeuralEmbeddingLayer(nn.Module):
    ...
    def forward(
        ...
        torch.FloatTensor, torch.LongTensor]:  # (bs, new_seq_len, hidden_size),  (bs, new_seq_len), (bs, new_seq_len)
        ...
        return self.dropout(x), spikes_timestamp
        ...
        if adapt:
            for i in range(len(self.embed_spikes)):
                if spike_log_init:
                    ...
                    self.embed_spikes[i][0].weight.data += log_scale.unsqueeze(1).expand_as(
                        self.embed_spikes[i][0].weight.data)
                else:
...
class NeuralAttention(nn.Module):
    ...
    def __init__(self, idx, hidden_size, n_heads, use_bias, dropout, use_rope=False, base=10000., max_F=1024):
        ...
        # RoPE parameters
        if use_rope:
            cos, sin = get_cos_sin(self.head_size, max_F, base=base, dtype=self.query.weight.dtype,
    ...
    def forward(
        ...
        out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask,
                                             ...
                                             is_causal=False)  # (B,n_heads,T,head_size)
...
class NeuralEncoderLayer(nn.Module):
    ...
    def __init__(self, idx, max_F, config: DictConfig):
        ...
        self.attn = NeuralAttention(idx, config.hidden_size, config.n_heads, config.attention_bias, config.dropout,
                                    config.use_rope, config.rope_theta, max_F)
...
class NeuralEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Context span mask. It's always causal for ndtgpt.
        self.context_forward = 0
        self.context_backward = -1
        ...
        self.layers = nn.ModuleList(
            [NeuralEncoderLayer(idx, config.embedder.max_F, config.transformer) for idx in range(self.n_layers)])
        self.out_norm = ScaleNorm(self.hidden_size ** 0.5) if config.transformer.use_scalenorm else nn.LayerNorm(
            self.hidden_size)
    ...
    def forward(
        ...
        # Embed neural data
        x, spikes_timestamp = self.embedder(spikes, spikes_timestamp, block_idx, date_idx)
        ...
        self_mask = torch.eye(T).to(x.device, torch.int64).expand(B, T,
                                                                  T)  # hack so that even padded spikes attend to themselves and avoid attention issues
...
class NDTGPT(nn.Module):
    ...
    def forward(
        ...
        # Compute the loss over non-padded outputs
        if self.method == "ssl":
            loss = (self.loss_fn(outputs, targets) * new_time_attn_mask.unsqueeze(2)).sum()
            n_examples = new_time_attn_mask.sum().to(loss.device, torch.long)  # only works for single session data
        elif self.method == "ctc":
            raise NotImplementedError("CTC not implemented yet")
        elif self.method == "sl":
            loss = self.loss_fn(outputs, targets).sum()
            n_examples = torch.Tensor([len(targets)]).to(loss.device, torch.long)
        else:
            raise Exception(f"Method {self.method} does not exist.")
